...
app.controller('searchCtrl',function($scope,$http){
    $http.get('/project/map/').success(function(data, status, headers, config) {
        $scope.items = data;
...
app.filter('searchFor', ['MapService', function(MapService){
    console.log(MapService)

    var searchFunction = function(item, searchString){
        if(item.name.toLowerCase().indexOf(searchString) !== -1){
            MapService.map.addLayer(MapService.markers[item.id]);
        } else if(item.tags.toString().toLowerCase().indexOf(searchString) !== -1) {
            MapService.map.addLayer(MapService.markers[item.id]);
        } else {
            MapService.map.removeLayer(MapService.markers[item.id]);
        }
    };
    
    return function(arr, searchString){
        if(!searchString){
            angular.forEach(MapService.markers, function(item){
                MapService.map.addLayer(item);
            });
            return arr;
        }
        var result = [];
        ...
        angular.forEach(arr, function(item){
            searchFunction(item, searchString);
            if(item.name.toLowerCase().indexOf(searchString) !== -1){
            result.push(item);
            }
        });
        ...
        $http.get('/project/map/').success(function(projects) {
            ...
            angular.forEach(projects, function(project, key) {
                ...
                if (project.publication) {
                    console.log(project.publication)
...
app.controller('StartMapController',['$scope','$document','$window','$timeout','$location','$anchorScroll','MapService',function($scope,$document,$window,$timeout,$location,$anchorScroll,MapService) {

    $scope.map = true;
    $scope.locked = false;
    $scope.mapService = MapService;
    markersss = $scope.mapService.markers;
    console.log($scope.mapService.markers);

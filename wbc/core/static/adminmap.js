...
function drawMap(){
    ...
    var button = $('<p/>', {
        html: '<input type="button" value="Adresse auf Karte markieren" class="btn btn-default"></input>'
    })
    ...
    .click(function(){
        ...
        var entities = [];
        var muncipality = $("#id_entities option:first-child").text(); // for now we assume there is only one
        ...
        if (address.length !== 0 && entities.length !== 0) {
            var url = 'http://nominatim.openstreetmap.org/search?format=json&limit=1&q=' + address + ' ' + entities.join(' ') + ' ' + muncipality;
            $.ajax({
                dataType: "json",
                url: url,

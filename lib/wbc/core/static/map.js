...
app.factory('MapService',['$http',function($http) {
    ...
    $http.get('/process/processsteps/').success(function(process_steps) {
        ...
        $http.get('/process/places/',{'params': {'active': true, 'internal': true}}).success(function(places) {

            // add points to map
            angular.forEach(places, function(place, key) {
                // get the first publication
                var publication = place.publications[0];

                // get the pk of the process step
                var process_step_id = publication.process_step.id;

                // get coordinates
                var lat = place.point[1];
                var lon = place.point[0];
                ...
                popuptext += '<p><a href="' + place.internal_link + '" >Details</a></p>';
